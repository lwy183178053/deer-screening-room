...
前端 E2E 同时检查随机种子分页（每页 20 条）、首页工作室筛选、Artplayer 桌面/手机布局、兑换码和鹿币页面的移动端溢出，以及播放器 DOM 中不存在广告和外部品牌链接。`VideoPlayer.test.ts` 额外验证移动浏览器从后台恢复时只重载一次当前 Range 地址，并在组件卸载后移除页面可见性监听。
