...
升级前先备份数据库与 `.env`，记录当前 Compose 容器 ID、启动时间和 Caddy 配置备份，再执行定向 `docker compose build` 与 `up -d`。数据库迁移在 Gateway 启动时按顺序执行；失败时 Gateway 不启动。升级后同时验证 TURN-only 与允许直连两种播放模式，并确认服务器既有容器没有重启。
