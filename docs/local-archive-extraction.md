...
Sample output is written to `E:\BaiduNetdiskDownload.av1-samples`; it never changes source media. Full/New mode requires FFmpeg with `av1_nvenc` and `ffprobe` on `PATH`, and uses AAC-LC 48 kHz stereo at a nominal 128 kbps; silence and low-complexity audio may report a lower measured average after a valid encode, so validation does not impose a minimum measured bitrate. If the media-node Compose service is not running, Full/New mode continues without attempting a stop or restart; a failed maintenance entry removes the lock it just created.
