...
- 媒体根目录仅对媒体节点容器提供写权限，用于同目录哈希改名和维护 `.deer-media-map.json`；浏览器和 Gateway 永远不接收 NAS 真实路径，媒体节点不拼接用户提供的 URL 路径。Windows 宿主机在容器扫描前使用标准化脚本处理无法被 Linux bind mount 枚举的长文件名。
