...
const redeemCodesLoadingMore = ref(false)
...
async function loadPublic() {
  try {
    ...
    catalogTotal.value = recommendationMode.value ? Math.min(videoBody.total ?? videos.value.length, 50) : (videoBody.total ?? videos.value.length)
...
async function refreshCatalog(reset = true) {
  ...
  try {
    ...
    catalogTotal.value = recommendationMode.value ? Math.min(body.total ?? videos.value.length, 50) : (body.total ?? videos.value.length)
  } catch (caught) {
...
async function loadRedeemCodes(reset = true) {
  const pageNumber = reset ? 1 : redeemPage.value + 1
  ...
  if (pageNumber > 1) params.set('page', String(pageNumber))
  if (!reset) redeemCodesLoadingMore.value = true
  try {
    const body = await api<RedeemCodePage>(`/api/v1/admin/redeem-codes?${params}`)
    const incoming = body.codes ?? []
    redeemCodes.value = reset ? incoming : [...redeemCodes.value, ...incoming]
  ...
  } finally { redeemCodesLoadingMore.value = false }
}
...
async function loadMoreRedeemCodes() { if (!redeemCodesLoadingMore.value && redeemCodes.value.length < redeemTotal.value) await loadRedeemCodes(false) }
    ...
    <template v-else>
      ...
      <main>
        ...
        <section v-else-if="activeView === 'account'" class="account-view">
          <header class="page-heading"><div><span>我的账户</span><h1>{{ account?.email }}</h1></div><button class="icon-text" type="button" @click="logout"><LogOut :size="17" />退出</button></header>
