...
func BuildBundle(data BundleData) ([]byte, error) {
	...
	files := map[string]string{
		...
		"README.txt":   "小鹿放映室媒体节点\n\n导入此 Docker 项目后，只需要选择一次 NAS 视频目录，将它映射到容器 /media，并启动项目。节点保留工作室目录，在扫描时把视频标准化为短哈希文件名，并在媒体根目录写入 .deer-media-map.json；网站仍显示原始标题。node.env 已包含本节点的一次性连接配置，请保留在 NAS 私有目录中。\n\n节点启动后，管理员页面会在 90 秒内显示在线；首次扫描完成后视频会出现在目录。\n",
	}
...
services:
  ...
  node:
    ...
    volumes:
      - ${DEER_MEDIA_HOST_PATH:-/CHANGE_ME}:/media:rw
