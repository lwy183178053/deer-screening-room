...
func main() {
	...
	defer stop()
	node, err := media.NewNode(media.NodeConfig{Name: cfg.NodeName, PublicURL: cfg.NodePublicURL, GatewayURL: cfg.GatewayURL, NodeAPIToken: cfg.NodeAPIToken, MediaRoot: cfg.MediaRoot, PosterRoot: cfg.PosterRoot})
	if err != nil {
		log.Fatal(err)
	}
